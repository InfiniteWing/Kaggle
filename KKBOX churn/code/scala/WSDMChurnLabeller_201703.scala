...
object WSDMChurnLabeller {
  def main(args: Array[String]) : Unit ={
    ...
    val data = spark
      ...
      .csv("/home/sars6608/KKBOXchurn/input/transactions_all.csv")
    ...
    val predictionCandidates = userExpire
      .filter(
        col("last_expire") >= lit(targetExpireStart) and col("last_expire") <= lit(targetExpireEnd)
      )
    ...
    resultSet.repartition(1).write.option("header",value=true).csv(targetDIR)
    spark.stop()
