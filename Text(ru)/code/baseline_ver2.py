...
while 1:
    ...
    # Handle ambiguous by class
    c = (line.split(',')[2]).replace('"','')
    if(c not in all_res):
        all_res[c] = dict()
    if arr[0] not in all_res[c]:
        all_res[c][arr[0]] = dict()
    if arr[1] not in all_res[c][arr[0]]:
        all_res[c][arr[0]][arr[1]] = 0
    all_res[c][arr[0]][arr[1]] += 1 
...
total = 0
changes = 0
out = open(os.path.join(SUBM_PATH, 'baseline_ver2_ext_ru_2.csv'), "w", encoding='UTF8')
out.write('"id","after"\n')
test = open(os.path.join(INPUT_PATH, "ru_test_2.csv"), encoding='UTF8')
line = test.readline().strip()

label_preds = pd.read_csv('../input/test_pred_2.csv')
labels = label_preds['predict'].values
...
while 1:
    ...
    c = labels[total]
    if line in res:
        if line in all_res[c]:
            srtd = sorted(all_res[c][line].items(), key=operator.itemgetter(1), reverse=True)
            out.write('"' + srtd[0][0] + '"')
            changes += 1
        else:
