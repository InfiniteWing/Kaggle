...
xgb_moedels = []
for i in range(2):
    train_X, valid_X, train_y, valid_y = train_test_split(df_train, target_train, test_size = 0.1, random_state = i+3228)     
    d_train = xgb.DMatrix(train_X, train_y)
    d_valid = xgb.DMatrix(valid_X, valid_y)
    # xgboost, cross-validation
    watchlist = [(d_train, 'train'), (d_valid, 'valid')]
    model = xgb.train(xgb_params, d_train, 80, watchlist, early_stopping_rounds=20, verbose_eval=10)
    xgb_moedels.append(model)
for i in range(2):
...
for i, model in enumerate(xgb_moedels):
...
for i, model in enumerate(moedels):
    ...
    pred[pred < 0] = 0.01
    preds += pred
preds /= (len(xgb_moedels) + len(moedels))
...
print('Total {} seconds'.format((end_datetime-start_datetime).total_seconds()))