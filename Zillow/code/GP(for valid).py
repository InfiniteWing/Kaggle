...
valid_2017 = pd.read_csv("../input/train_valid_2017.csv")
...
valid_2017 = valid_2017.merge(properties, how='left', on='parcelid')
train['month'] = pd.DatetimeIndex(train['transactiondate']).month
valid_2017['month'] = -1
test['month'] = -1
logerrors = train.logerror.ravel()
y_valid = valid_2017['logerror'].values
...
for c in highcardinality:
    valid_2017.insert(1,'loo'+c, ProjectOnMedian(train,
                                           valid_2017,c))
valid_2017.drop(highcardinality,inplace=True,axis=1)
...
for c in feats:
    ...
    valid_2017[c] = valid_2017[c].fillna(train[c].median())
...
ss = StandardScaler()
ss.fit(pd.concat([xtrain[features],valid_2017[features]]))
xtrain[features] = ss.transform(xtrain[features])
valid_2017[features] = ss.transform(valid_2017[features])
...
testpreds = GP(valid_2017)
from sklearn.metrics import mean_absolute_error

valid_score = mean_absolute_error(y_valid, testpreds)

print('Valid MAE =', valid_score)


output = pd.DataFrame({'valid_pred': testpreds})
output.to_csv('valid_GP.csv', index=False, float_format = '%.5f')
